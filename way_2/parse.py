...
def rubi_rule_parser(fullform, header=None, module_name='rubi_object'):
    ...
    cons_index =0
    cons = ''
    input =['linear_product.txt', 'binomial_product.txt', 'quadratic_product.txt']
    output =['linear_products.py', 'binomial_products.py', 'quadratic_products.py']
    for index in range(0, 3):
        with open(input[index], 'r') as myfile:
        ...
        for i in parse_full_form(fullform): # separate all rules
            if i[0] == 'RuleDelayed':
                rules.append(i)
        parsed = downvalues_rules(rules, header, cons_dict, cons_index)
